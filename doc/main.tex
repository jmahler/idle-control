...
% {{{ Control Law, Full Feedback, No Prediction
\clearpage
...
\begin{tikzpicture}[>=triangle 60,
	...
	\node[sumxpxp] 	(sum1)	[]					{};
	\node[gain]  	(R)		[right=of sum1] 	{$R$};
	\node[gain]  	(Gz)	[right=of R] 		{$x(k + 1) = \Phi x + \Gamma u$};
	\node[sumxpxn]	(sum2)	[right=of Gz]		{};
	\node[gain]		(H)		[right=of sum2]		{$H$};
	\node[gain] 	(K)		[below=of Gz] 		{$-K$};
	...
	\draw [->] (Gz.east) -- (sum2.west);
	\draw [<-] (sum2.north) -- ++(0,10mm) node[above]{$\text{rpm}$};
	\draw [->] (sum2.east) -- ++(7mm,0) node[above,xshift=0mm]{$x$}
		-| ++(0,-10mm)
		|- (K.east);
	\draw [->] (sum2.east) -- (H.west);
	\draw [->] (H.east) -- ++(10mm,0) node[above,xshift=-3mm]{$y$};
	\draw [<-] 
		(sum1.west) -- ++(-10mm,0) node[above,xshift=3mm]{$u$}
		|- (K.west);
\end{tikzpicture}
...
\begin{tikzpicture}[>=triangle 60,
	...
	\node[gain]  	(Gz)	[] 		{$x(k + 1) = \Phi x + \Gamma u$};
	\node[gain] 	(K)		[below=of Gz] 	{$-K$};
	\node[gain]		(H)		[right=of Gz]	{$H$};

	\draw [->]
	 		(Gz.east) -- ++(7mm,0)
			-| ++(0,-5mm)
			|- (K.east);
	\draw [->] (Gz.east) -- (H.west) node[above,xshift=-7mm]{$x$};
	\draw [->] (H.east) -- ++(10mm,0) node[above,xshift=-3mm]{$y$};
...
\begin{tikzpicture}[>=triangle 60,
	...
	\node[gain]  	(R)		[] 				{$R$};
	\node[sumpp]	(sum1)	[right=of R]	{};
	\node[gain]  	(Gz)	[right=of sum1]	{$x(k + 1) = \Phi x + \Gamma u$};
	\node[sumxpxn]	(sum2)	[right=of Gz]	{};
	\node[gain] 	(K)		[below=of Gz] 	{$-K$};
	\node[gain]		(H)		[right=of sum2]	{$H$};

	\draw [->] ++(-15mm,0) node[left]{$\text{rpm}$} -- (R.west);
	\draw [->] (R.east) -- (sum1.west);
	\draw [->] (sum1.east) -- (Gz.west);
	\draw [<-] (sum2.north) -- ++(0,10mm) node[above]{$\text{rpm}$};
	\draw [->] (Gz.east) -- (sum2.west);
	\draw [->]
	 		(sum2.east) -- ++(7mm,0)
			-| ++(0,-5mm)
			|- (K.east);
	\draw [->] (sum2.east) -- (H.west) node[above,xshift=-7mm]{$x$};
	\draw [->] (H.east) -- ++(10mm,0) node[above,xshift=-3mm]{$y$};
	\draw [<-]
	 		(sum1.south)
			|- (K.west);
...
\begin{figure}[htbp!]
\begin{center}
\includegraphics[scale=0.7]{../matlab/cl_fonp03}
\end{center}
...
\lstinputlisting[
	caption={Matlab source of rpmtime() function.} 
	]{../matlab/rpmtime.m}
...
\lstinputlisting[
	caption={Matlab source used to plot the Control Law with no predicition
	if sum block is ignored (Figure \ref{fig:cl_fonp03}).},
	label=lst:cl_fonp03,
	]{../matlab/cl_fonp03.m}
