...
It can be seen in Figure \ref{fig:nodelta} there is a beneficial
side effect that makes this trivial.  The transform that converts
from steady state to a delta cancels
with the transform that converts from a delta to steady state
\footnote{If the torque input was non zero the transforms would not
cancel.}.  Therefore the final transform is still Equation \ref{eq:lems}.
...
rpm near 900.
The Matlab script is provided in Appendix \ref{app:em_matlab}.
\label{fig:em_plot}
\end{figure}


% see octave/engine_model.m for these calculations
